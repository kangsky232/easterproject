...
摄像头流、真实短信/APP 通道和 MQTT 广播仍是外部适配点；华为云 MQTT 入站遥测已经实现。3D 社区内置 9 张由 AI 生成的过道、电梯厅、楼梯间、地下连廊和预警演示图，按楼栋与楼层稳定分配并明确标注为模拟画面；它们不代表已经接入真实摄像头或视觉模型。当前项目内置的是可本地验收的复核规则、通知模拟和安全知识问答。
