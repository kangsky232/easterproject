...
- 设备总览、设备列表和浓度趋势；实时模式展示最近 120 条原始数据，另提供 24 小时、7 天和 30 天聚合视图
- 告警查询、确认与处置
- 设备绑定、编辑、阈值设置与解绑
- 向当前选中的设备创建广播指令
- 告警复核、误报标记，以及由新告警触发的 APP / 短信模拟通知记录
- 基于告警上下文的安全知识问答（优先使用本地 RAG 服务，服务不可用时安全降级）
- 每 10 秒轮询后端并刷新实时曲线；连接失败时退避到最长 60 秒，后端当前未提供 WebSocket 推送接口
- “模拟告警”通过 `POST /api/telemetry` 上报一次超阈值数据

浓度在界面统一显示两位小数。后端和数据库会保留新上报数据的小数，但旧版本已经截断的小数无法恢复。

摄像头流、真实短信/APP 通道和 MQTT 广播仍是外部适配点；华为云 MQTT 入站遥测已经实现。当前项目内置的是可本地验收的复核规则、通知模拟和安全知识问答。
