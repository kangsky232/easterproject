...
AI 视觉功能默认启用，但后端每次启动后的自动巡检状态默认暂停；有 `VISION_REVIEW` 权限的账号在 3D 社区页点击“开始巡检”后，才会每 15 秒轮换模拟图片，点击“暂停巡检”后停止新的自动分析和钉钉识别告警。在 `.env.vision.local` 中写入 `DEEPSEEK_API_KEY=<你的密钥>` 后，启动脚本会切换到 DeepSeek Vision；未配置 Key 时界面与事件使用 `SIMULATION_FALLBACK`。疑似结果会进入 `vision_event`，钉钉已配置时自动推送给绑定员工，再由有 `VISION_REVIEW` 权限的账号人工研判。当前输入仍是静态模拟图，不是真实摄像头。
