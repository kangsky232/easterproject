...
- 启用 DeepSeek 时通过机密管理注入 `DEEPSEEK_API_KEY`，验证配额、超时和外部图片可达性；未配置时验收界面、API 和钉钉消息均明确标注模拟规则与非真实摄像头。
- RAG 镜像当前使用 Flask 开发服务器；正式部署前改为 Gunicorn 等生产 WSGI 服务，并配置并发、超时、优雅退出和健康检查。
- 使用 HTTPS 域名验证登录、改密后旧 Token 失效、角色权限、设备令牌、告警全流程和错误响应。
- 已有数据库按版本依次执行 `docs/migrations/20260826_decimal_concentration.sql`、`docs/migrations/20260826_extended_sensor_metrics.sql`、`docs/migrations/20260828_role_workspace_3d_map.sql`、`docs/migrations/20260831_hazard_workflow.sql`、`docs/migrations/20260831_notification_audit.sql` 和 `docs/migrations/20260901_vision_patrol.sql`，并验证小数与扩展遥测、趋势聚合、告警、角色工作区、3D 地图位置、隐患闭环、通知核查及视觉事件人工研判。
...
2. 复制 `.env.example` 为 `.env`，替换所有 `REPLACE_*`，并将 `CORS_ALLOWED_ORIGINS` 改为实际 HTTPS 域名。如需 DeepSeek 分析，填写 `DEEPSEEK_API_KEY`；Compose 已把全部 `VISION_*` 和 DeepSeek 配置传入后端。
...
- 当前 APP 仅代表本地通知中心记录，SMS 保持 `PENDING` 且不会实际发送。视觉巡检可以调用 DeepSeek 分析图片，但当前帧仍来自静态模拟轮播；接入真实摄像头、完成隐私与效果评估前，不能把它当成生产火灾报警或现场证据。
