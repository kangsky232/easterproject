...
| 角色 | 说明 | 前端可提供的主要操作 |
| --- | --- | --- |
| `RESIDENT` | 居民 | 实时监控、3D 社区、智能问答；全部只读 |
| `FIREFIGHTER` | 消防人员 | 实时监控、3D 社区、通知记录、广播管理、智能问答；可处置告警和下发广播 |
| `COMMUNITY_ADMIN` | 社区管理员 | 实时监控、3D 社区、设备管理、通知记录、广播管理、智能问答；可管理设备与地图位置 |
| `SYSTEM_ADMIN` | 系统管理员 | 社区管理员全部页面 + 用户管理；拥有全部业务管理操作 |

登录后前端先调用 `/api/auth/me` 同步数据库中的最新角色，再使用 `/api/auth/workspace` 返回的可见模块生成导航。可见模块按“监控中心、事件处置、资源管理、智能辅助”归类，空分类不会显示；无权模块不会挂载。工作区加载失败时仅保留实时监控、3D 社区、智能问答三个安全只读页面，不继承上一账号的管理权限。后端仍会强制校验，收到 `403` 时应显示“没有操作权限”。居民不能读取通知与广播记录；开发态“模拟告警”仅向社区管理员和系统管理员显示，生产模式隐藏。
...
地图场景的 `buildings` 用于绘制 3D 楼栋，`devices` 包含 `status`（`ONLINE` / `OFFLINE` / `ALARM`）、`alertSeverity`、楼层/房间、坐标和最新六类遥测。前端把可见立面按楼层切分为点击区域，楼栋、楼层按钮、设备列表和右侧详情联动；地图每 3 秒刷新时保留用户当前楼层和管理员尚未提交的位置表单。居民与消防员看到只读地图，小区管理员和系统管理员显示位置编辑表单。前端修改位置后必须重新拉取场景，不在本地伪造保存成功。

当前过道主视角、公共区域和 AI 预警复核共使用 9 张静态演示图片。前端根据 `buildingCode + floorNo` 稳定映射三类互不重复的机位画面，并在界面明确标注“模拟画面 / 尚未接入真实摄像头”。这些图片不属于 `/api/map/scene` 返回值，也不能作为真实视觉复核结果；后续接入摄像头时应补充摄像头目录、流地址/回放地址、访问鉴权和模型结果接口。
