...
居民、消防员、小区管理员和系统管理员登录后会获得不同的模块与操作入口；可见模块按“监控中心、事件处置、资源管理、智能辅助”归类，后端仍是权限最终边界。3D 地图支持直接点击楼栋立面和楼层，联动查看本层设备、状态及 AI 生成的过道监控演示图；小区管理员和系统管理员可修改设备位置并同步写入 MySQL。演示图不是实时摄像头视频。
