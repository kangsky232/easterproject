...
3D 社区页新增“AI 视觉实时巡检”：服务启动后默认暂停，消防员、小区管理员或系统管理员点击“开始巡检”后，15 个模拟机位才按洗牌顺序轮换，每轮全部出现一次且相邻不重复；点击“暂停巡检”后不再自动分析或发送新的钉钉告警。其中 10 张正常、5 张疑似烟火。配置 `DEEPSEEK_API_KEY` 后由 `deepseek-v4-flash-vision-exp` 分析画面；没有密钥时明确显示并使用内置场景规则，绝不把规则结果冒充为模型结论。超过置信度阈值的疑似事件会写入 MySQL，并向已绑定员工发送钉钉单聊；有复核权限的工作人员必须填写依据，才能一次性确认为火情或排除误报。当前图片仍是静态演示素材，不是真实摄像头证据。
